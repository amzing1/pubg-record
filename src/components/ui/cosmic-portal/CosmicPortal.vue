<template>
  <div :class="['relative overflow-hidden h-full w-full', props.containerClass]">
    <canvas ref="canvasRef" :class="['absolute inset-0 h-full w-full block', props.class]" />
  </div>
</template>
...
const props = withDefaults(defineProps<Partial<PortalParams> & { class?: string; containerClass?: string }>(), {
  portalComplexity: 4,
  crystalCount: 12,
  primaryColor: '#9b59b6',
...
function animate() {
  ...
  portalMaterials.forEach((shader) => {
    if (shader.uniforms) {
      ...
      if (shader.uniforms.dimensionShift) shader.uniforms.dimensionShift.value = params.value.dimensionShift
    }
  })
